#!/usr/bin/env python
"""
Run the binary "epidemic this quarter" classifier's nested CV, evaluate it
against the existing nivel_inc/sustained_rt regime-proxy rules on the same
quarterly-grain OOF rows, and report the comparison. Structurally independent
...
if __name__ == "__main__":
    ...
    fold_predictions_reg = pd.read_csv(outputs_dir / "fold_predictions.csv", parse_dates=["quarter_start"])
    ...
    print("\n=== Proxy comparison (fair, quarterly grain) ===")
