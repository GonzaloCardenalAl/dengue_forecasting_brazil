...
if __name__ == "__main__":
    ...
    # Parallel horizon-aware deliverable, computed alongside (not instead of)
    # the flat-CI forecast above -- artifact["horizon_quantiles"] is only
    # populated when run_autoregressive_cv.py has been run for this run dir
    # (see training/final_train.py); otherwise this whole block is a no-op,
    # so older run dirs are unaffected.
    ...
    if horizon_quantiles is not None:
        ...
        print(f"\nHorizon-aware quarterly forecast (Q1 narrower, Q4 wider CI) saved to "
              f"{run_dir / 'final_quarterly_forecast_horizon_aware.csv'}")
    else:
        print(f"\nNo horizon_quantiles in {model_path.name} -- skipping the parallel "
